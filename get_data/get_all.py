...
for file_x in os.listdir(os.path.join(path, 'data_2', '大峡谷隧道监测数据')):
    input = os.path.join(path, 'data_2\\大峡谷隧道监测数据\\%s' % (file_x))
    for bb in os.listdir(os.path.join(path, 'data_2', '大峡谷隧道监测数据',file_x)):
        ...
        for tag, df in df.items():
            ...
            if (tag[0] == 'K') | (tag[0] == 'k') | (tag[:2] == 'ZK') | (tag[:2] == 'zk'):
                tag_list.append(tag)
                # print(tag,df)
